...
{% endhighlight %}
Then, the regret bound grows linearly with time `t` but if they follow the `Upper Confidence Bound` algorithm <meta content="0; url=https://www.youtube.com/watch?v=eM6IBYVqXEA" /> as 
{% highlight python %}
def ucb_algorithm(t):
